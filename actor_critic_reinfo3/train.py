...
def pred_input(conf):
    # 주어진  시작일 이전 100봉을 포함하여 수익률 결과 파일 생성후 100봉씩 수익률 추이를 input으로 append

    # 데이터 전처리
    df = pd.read_csv(conf.df0_path, encoding='euc-kr')
    start_index = df.loc[df['date'] >= conf.start_time].index.min()
    conf.start_time = df['date'].values[start_index - 99]
    ...
    # 이전 100봉의 수익률 추이를 하나의 input으로 train_step 진헹
    inputs = []
    for i in range(99, len(results_df)):
        # 이전 100봉의 예측값, 종가 추출
        pred = results_df['result'].values[i-99:i+1]
        close = results_df['close'].values[i-99:i+1]
        ...
        for j in range(len(pred) - 1):
            if pred[j] == 1:
                input.append((1 - close[j + 1] / close[j]) * 100)
                state = 1
            elif pred[j] == 2:
                input.append((close[j + 1] / close[j] - 1) * 100)
                state = 2
            elif state == 1:
                input.append((1 - close[j + 1] / close[j]) * 100)
            elif state == 2:
...
#@tf.function
def train_step(conf, actor_critic, inputs):
    """
    conf: 모든 정보
    ...
    with tf.GradientTape() as tape:
        ...
        action_probs_t, values = actor_critic(inputs)
        values = tf.reshape(values, [-1])
        actions = tf.argmax(action_probs_t, 1)

        # ep.predict에서 각 봉별 수익률에 의한 rewards 출력
        rewards = create_rewards(conf, actions)
...
def create_rewards(conf, actions):
...
def train(conf, model_name, rwd_th, max_eps, min_eps):
    ...
    for i in t:
        ...
        # conf의 start_time과 end_time은 pred_input()에서 변경되므로 save했다가 다시 복원
        save_start_time = conf.start_time
        save_end_time = conf.end_time

        # 각 종 ,data path 설정
        data.set_path(conf, conf.dir)
        inputs = pred_input(conf)

        conf.start_time = save_start_time
        conf.end_time = save_end_time
        ...
        episode_reward = train_step(conf, actor_critic, inputs)
...
def test(conf, model_name):
    ...
    # start_time이 pred_input과정에서 바뀌므로 save했다가 복원 필요
    save_start_time = conf.start_time

    # actor-critic model에서 reinfo action과 value 출력
    input = pred_input(conf)
    conf.start_time = save_start_time # start_time 복원
    action_probs_t, values = actor_critic(input)
    actions = tf.argmax(action_probs_t, 1)

    # action 적용전 예측 결과 파일 생성
    conf.result_path = 'test_results.csv'
    conf.start_time = save_start_time # start_time 복원
    p = ep.predict(conf)
    print("적용전 수익률: ", str(p))

    # action을 make_reinfo2에 적용하여 새로운 예측과 수익률 생성
    r = pd.read_csv(conf.result_path, encoding='euc-kr').values
    ep.mr.ths = actions
    pred = r[:, 1]
    pred_results = r[:, :6]
...
class env:
    ...
    def __init__(self, conf):
        ...
        self.state = tf.constant(np.zeros(100), dtype=tf.float32)
        self.cnt = 0
        self.max_cnt = 100
    ...
    def step(self, action: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
        ...
        if self.cnt > self.max_cnt - 1:
            return self.state, tf.reduce_mean(self.state)

        mr.th = action / tf.constant(20)
        mr.pred_term = self.conf.pred_term
        mr.target_type = self.conf.target_type
        pred = mr.reinfo(self.inputs[self.cnt-1, :, 1], self.inputs[self.cnt-1, :, :6], self.inputs[self.cnt-1, 0, 0],
                         self.conf.reinfo_width)
        self.inputs[self.cnt-1, :, 1] = np.array(pred)

        close = self.inputs[self.cnt-1, :, 5]

        state = 0
        reward_input = []
        for j in range(len(pred) - 1):
            if self.inputs[self.cnt-1, j, 0][11:16] >= '15:00':
                state = 0
                reward_input.append(0)
            elif pred[j] == 1:
                reward_input.append((1 - close[j + 1] / close[j]) * 100)
                state = 1
            elif pred[j] == 2:
                reward_input.append((close[j + 1] / close[j] - 1) * 100)
                state = 2
            elif state == 1:
                reward_input.append((1 - close[j + 1] / close[j]) * 100)
            elif state == 2:
        ...
        # reward는 현재 step의 마지막 봉과 다음 step의 마지막 봉 사이의 rate
        if state == 1:
            last_reward = (1 - self.inputs[self.cnt, -1, 5] / close[-1]) * 100
        elif state == 2:
            last_reward = (self.inputs[self.cnt, -1, 5] / close[-1] - 1) * 100
        else:
            last_reward = 0

        reward_input.append(last_reward)

        self.state = tf.convert_to_tensor(np.array(reward_input))
        reward = tf.convert_to_tensor(last_reward)

        return self.state, reward

    def reset(self, conf) -> tf.Tensor:

        # 시작일로부터 100봉이전에서 시작하여 100봉씩 예측 결과 파일을 하나의 input으로 차례로 저장
        self.conf = conf
        save_start = conf.start_time
        start_index = self.df.loc[self.df['date'] >= conf.start_time].index.min()

        # 데이터 전처리
        conf.start_time = self.df['date'].values[start_index - 99]
        data.preprocessing(conf)
        ...
        # 이전 100봉의 result를 하나의 input으로 train_step 진헹
        inputs = []
        for i in range(99, len(results_df)):
            inputs.append(results_df[i-99:i+1].values)

        self.conf.start_time = save_start
        conf.start_time = save_start

        self.max_cnt = len(results_df) - 99

        self.inputs = np.array(inputs)

        self.cnt = 0

        pred = self.inputs[self.cnt, :, 1]
        close = self.inputs[self.cnt, :, 5]
...
def train3(conf, model_name, rwd_th, max_eps, min_eps):
    ...
    for i in t:
        ...
        episode_reward = train_step3(init_state, actor_critic, optimizer, gamma, env.max_cnt)
...
if __name__ == "__main__":
    ...
    model_name = '알고리즘트레이딩2-1/2022-01-01~2025-01-31_' + 'version' + str(version) + "_rwd_th" + str(rwd_th)

    trained_model = ["15HL", "40HL", "5C", 0.1, 20, 30, 0.002]
    ...
    if mode == 'train':
        ...
        conf.start_time = '2024/01/01/09:00'
        ...
        if version == 0:
            ...
            train3(conf, model_name, rwd_th, max_eps, min_eps)

    else:
        conf.start_time = '2025/02/17/09:00'
        conf.end_time = '2025/02/25/15:00'
        conf.last_train = '2025-01-31'
