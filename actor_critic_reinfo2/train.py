...
# randon으로 선택된 앙상블 모델의 봉과 봉사이의 reinfo action에 따른 수익률이고 주어진 기간에서의 합이 return이 된다.
# vesrsion1 과의 차이는 모델 정보가 input으로 추가된다는 것이다.
...
def train(conf, model_name, rwd_th):
    ...
    for i in t:
        ...
        conf.loss_cut = random.randint(2, 20) / 2000
        ...
        # # 앙상블의 기본모델 타입 앙상블 pred_term, width input에 첨가
        input = add_input(conf, input)
...
def add_input(conf, input):
    # 앙상블의 기본모델 타입 앙상블 pred_term, width input에 첨가
    add_data = []

    term, type, _, _ = model.parse(conf.selected_model_types[0])
    if type == 'C':
        type = 0
    elif type == 'HL':
        type = 1
        ...
        type = 2
    add_data.append(int(term))
    add_data.append(type)
    term, type, _, _ = model.parse(conf.selected_model_types[1])
    if type == 'C':
        type = 0
    elif type == 'HL':
        type = 1
    ...
    add_data.append(int(term))
    add_data.append(type)
    term, type, _, _ = model.parse(conf.selected_model_types[2])
    if type == 'C':
        type = 0
    elif type == 'HL':
        type = 1
    ...
    add_data.append(int(conf.pred_term))
    add_data.append(int(conf.reinfo_width))
    add_data.append(conf.loss_cut)

    add_datas = []
    for j in range(len(input)):
        add_datas.append(add_data)
    input = np.concatenate([input, np.array(add_datas)], axis=1)
...
if __name__ == "__main__":
    ...
    if mode == 'train':
        ...
        conf.start_time = '2023/11/01/09:00'
        conf.end_time = '2024/10/31/15:00'

        train(conf, '알고리즘트레이딩2-1/2023-11-01~2024-10-31(3)', 3)
    else:
        conf.input_size = 83
        conf.dir = 'H:/알고리즘트레이딩2-1'
        model_name = '알고리즘트레이딩2-1/2023-11-01~2024-10-31(3)'

        conf.start_time = '2024/11/01/09:00'
        conf.end_time = '2024/11/15/15:00'
        conf.last_train = '2024-10-31'

        trained_model = ['10P', '30P', '5P', 0.13, 8, 34, 0.007437]
